...
def perform_lab(path, standard_deviation, kernel_size, bound_path):
...
def convolution(img, kernel):
    size = len(kernel)
    matrix = calculate_matrix_with(img)

    start_x = size // 2
    start_y = size // 2

    for i in range(start_x, len(matrix)-start_x):
        for j in range(start_y, len(matrix[i])-start_y):
            matrix[i][j] = calculate_value(img, size, kernel, i, j)

    return matrix


def calculate_matrix_with(img):
...
def calculate_value(img, size, kernel, i, j):
    value = 0
    for k in range(-(size // 2), size // 2 + 1):
        for l in range(-(size // 2), size // 2 + 1):
            value += img[i + k][j + l] * kernel[k + (size // 2)][l + (size // 2)]
    return value
...
def start_point():
    perform_lab('/Users/snowlukin/Desktop/DigitalMediaClass/Resources/lab2_img1.jpg', 10, 3, 15)
    perform_lab('/Users/snowlukin/Desktop/DigitalMediaClass/Resources/lab2_img1.jpg', 10, 11, 6)
    perform_lab('/Users/snowlukin/Desktop/DigitalMediaClass/Resources/lab2_img1.jpg', 100, 33, 55)
    while True:
